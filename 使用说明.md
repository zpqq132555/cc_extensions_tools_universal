# Extensions Tools - Cocos Creator 跨版本通用插件

## 📖 项目简介

这是一个同时兼容 **Cocos Creator 2.x** 和 **3.x** 版本的扩展插件，采用适配器模式实现跨版本 API 统一，让开发者无需关心版本差异即可编写扩展功能。

### 核心特性

- ✅ **跨版本兼容**：自动检测 Cocos Creator 版本，使用对应的 API
- ✅ **统一接口**：提供一致的开发体验，无需编写版本判断代码
- ✅ **模块化设计**：核心功能与业务逻辑分离，易于扩展维护
- ✅ **TypeScript 支持**：完整的类型定义，开发更安全高效
- ✅ **Inspector 扩展**：支持在资源管理器中显示自定义属性面板

---

## 🚀 快速开始

### 1. 安装插件

将插件文件夹复制到项目的 `packages` 目录下：

```
your-project/
└── packages/
    └── extensions_tools_universal/
```

### 2. 编译插件

在插件目录下运行：

```bash
npm install
npm run build
```

或分别编译 v2/v3 版本：

```bash
npm run build:v2  # 编译 Cocos 2.x 版本
npm run build:v3  # 编译 Cocos 3.x 版本
```

### 3. 启用插件

**Cocos Creator 2.x:**
- 顶部菜单：`扩展 → 扩展管理器`
- 找到 `extensions_tools` 并启用

**Cocos Creator 3.x:**
- 顶部菜单：`扩展 → Extension Manager`
- 找到 `extensions_tools` 并启用

### 4. 重载插件（可选）

**Cocos Creator 2.x:**
- `开发者 → 重新加载插件`

**Cocos Creator 3.x:**
- `开发者 → 重新加载扩展`

---

## 🔧 功能说明

### 1. 工具面板

#### 打开方式

**Cocos Creator 2.x:**
- 菜单：`扩展 → Extensions Tools → Open Panel`

**Cocos Creator 3.x:**
- 菜单：`面板 → Extensions Tools → Open Panel`

#### 面板功能

- 显示插件状态和计数器
- 测试主进程与面板通信
- 按钮交互示例

### 2. Bundle 路径映射工具

自动扫描项目中的资源 Bundle，生成路径映射配置。

#### 使用方式

**Cocos Creator 2.x:**
- 菜单：`扩展 → Extensions Tools → 映射资源名称路径`

**Cocos Creator 3.x:**
- 菜单：`工具 → Extensions Tools → 映射资源名称路径`

#### 功能说明

- 自动扫描 `assets` 目录下的所有 Bundle
- 生成资源路径映射关系
- 便于在代码中引用资源路径

### 3. 资源目录说明（Inspector 扩展）

在资源管理器中选中文件夹时，可在属性检查器中显示该文件夹的说明文档。

#### 使用方法

1. **创建说明文档**
   
   在目标文件夹内创建一个隐藏的 Markdown 文件，命名规则：`.{文件夹名称}.md`
   
   例如，对于文件夹 `assets/game/scripts`，创建文件：
   ```
   assets/game/scripts/.scripts.md
   ```

2. **编写文档内容**
   
   在 `.scripts.md` 中编写说明文字：
   ```markdown
   ## 脚本目录
   
   存放游戏核心逻辑脚本
   
   - GameManager.ts - 游戏管理器
   - PlayerController.ts - 玩家控制器
   - EnemyAI.ts - 敌人 AI
   ```

3. **查看说明**
   
   在资源管理器中点击选中该文件夹，右侧属性检查器会显示：
   ```
   📁 资源目录说明
   ┌──────────────────────┐
   │ ## 脚本目录          │
   │                      │
   │ 存放游戏核心逻辑... │
   └──────────────────────┘
   ```

#### 显示位置

- **Cocos Creator 2.x**: 选中文件夹后，在右侧 `属性检查器（Properties）` 面板显示
- **Cocos Creator 3.x**: 选中文件夹后，在右侧 `检查器（Inspector）` 面板显示

---

## 📁 项目结构

```
extensions_tools_universal/
├── package.json              # 主配置文件（v2 格式）
├── package.v2.json           # Cocos 2.x 专用配置（未使用）
├── package.v3.json           # Cocos 3.x 专用配置（未使用）
├── tsconfig.json             # TypeScript 基础配置
├── tsconfig.v2.json          # v2 编译配置
├── tsconfig.v3.json          # v3 编译配置
├── README.md                 # 项目说明
├── 使用说明.md              # 本文档
│
├── src/                      # 源代码目录
│   ├── index.ts              # 统一导出入口
│   ├── main.ts               # 插件主入口
│   │
│   ├── core/                 # 核心框架
│   │   ├── interfaces.ts     # 跨版本抽象接口定义
│   │   ├── version-detector.ts # 版本检测器
│   │   ├── factory.ts        # 适配器工厂
│   │   └── base-plugin.ts    # 基础插件类
│   │
│   ├── adapters/             # 版本适配器
│   │   ├── v2/               # Cocos 2.x 适配器
│   │   │   ├── editor-adapter.ts
│   │   │   ├── panel-adapter.ts
│   │   │   ├── ipc-adapter.ts
│   │   │   ├── asset-adapter.ts
│   │   │   └── scene-adapter.ts
│   │   │
│   │   └── v3/               # Cocos 3.x 适配器
│   │       ├── editor-adapter.ts
│   │       ├── panel-adapter.ts
│   │       ├── ipc-adapter.ts
│   │       ├── asset-adapter.ts
│   │       └── scene-adapter.ts
│   │
│   ├── business/             # 业务功能模块
│   │   ├── BundlePath.ts     # Bundle 路径映射工具
│   │   └── my-panel.ts       # 面板实现
│   │
│   ├── asset_directory/      # Inspector 扩展
│   │   ├── index.ts          # v3 版本实现
│   │   └── index.v2.ts       # v2 版本实现
│   │
│   └── panels/               # 面板定义
│       └── default/
│           └── index.ts      # 默认面板
│
├── dist/                     # 编译输出目录
│   ├── v2/                   # v2 编译产物
│   └── v3/                   # v3 编译产物
│
├── i18n/                     # 国际化
│   ├── zh.js                 # 中文
│   └── en.js                 # 英文
│
└── scripts/                  # 构建脚本
    └── install.js            # 安装脚本

```

---

## 🛠️ 开发指南

### 核心设计模式

#### 1. 抽象接口层（interfaces.ts）

定义所有跨版本通用的接口，屏蔽版本差异：

```typescript
export interface IEditorAdapter {
    getVersion(): string;
    log(message: string, ...args: any[]): void;
    getProjectPath(): string;
}

export interface IPanelAdapter {
    open(panelName: string): void;
    close(panelName: string): void;
}

export interface IIPCAdapter {
    sendToMain(channel: string, ...args: any[]): void;
    sendToPanel(panelName: string, channel: string, ...args: any[]): void;
}
```

#### 2. 版本检测（version-detector.ts）

自动检测当前运行的 Cocos Creator 版本：

```typescript
export class VersionDetector {
    static detect(): CocosVersion {
        // 自动检测逻辑
    }
    
    static isV2(): boolean { /* ... */ }
    static isV3(): boolean { /* ... */ }
}
```

#### 3. 适配器工厂（factory.ts）

根据版本动态创建对应的适配器实例：

```typescript
export class AdapterFactory {
    static getInstance(): IAdapterManager {
        const version = VersionDetector.detect();
        // 返回对应版本的适配器
    }
}
```

#### 4. 基础插件类（base-plugin.ts）

提供统一的插件开发基类：

```typescript
export abstract class BasePlugin {
    protected get editor(): IEditorAdapter { /* ... */ }
    protected get panel(): IPanelAdapter { /* ... */ }
    protected get ipc(): IIPCAdapter { /* ... */ }
    
    abstract load(): void;
    abstract unload(): void;
}
```

### 添加新功能

#### 1. 创建业务模块

在 `src/business/` 下创建新文件：

```typescript
// src/business/MyFeature.ts
import { getEditor, getPanel } from '../index';

export class MyFeature {
    static run() {
        const editor = getEditor();
        editor.log('My feature is running!');
        
        // 使用统一接口，无需关心版本
        const panel = getPanel();
        panel.open('my-panel');
    }
}
```

#### 2. 在主插件中注册

在 `src/main.ts` 中添加消息处理：

```typescript
class ExtensionsToolsPlugin extends BasePlugin {
    messages = {
        'my-feature': () => this.handleMyFeature(),
    };
    
    methods = {
        myFeature: () => this.handleMyFeature(),
    };
    
    private handleMyFeature() {
        MyFeature.run();
    }
}
```

#### 3. 配置菜单项

**package.json (v2):**
```json
{
    "main-menu": {
        "i18n:MAIN_MENU.package.title/extensions_tools/my-feature": {
            "message": "extensions_tools:my-feature"
        }
    }
}
```

**package.v3.json (v3):**
```json
{
    "contributions": {
        "menu": [{
            "path": "i18n:menu.develop/My Feature",
            "message": "myFeature"
        }],
        "messages": {
            "myFeature": {
                "methods": ["myFeature"]
            }
        }
    }
}
```

### 创建自定义面板

参考 `src/business/my-panel.ts` 和 `src/panels/default/index.ts` 的实现方式。

---

## 🎯 使用示例

### 示例 1：获取编辑器信息

```typescript
import { getEditor, isV2, isV3 } from './index';

const editor = getEditor();
console.log('版本:', editor.getVersion());
console.log('项目路径:', editor.getProjectPath());
console.log('是否 v2:', isV2());
console.log('是否 v3:', isV3());
```

### 示例 2：打开面板

```typescript
import { getPanel } from './index';

const panel = getPanel();
panel.open('extensions_tools');
```

### 示例 3：发送消息

```typescript
import { getIPC } from './index';

const ipc = getIPC();
ipc.sendToPanel('extensions_tools', 'hello', { data: 'test' });
ipc.broadcast('global-event', { message: 'hello world' });
```

### 示例 4：操作资源

```typescript
import { getAsset } from './index';

const asset = getAsset();
const uuid = asset.urlToUuid('db://assets/scripts/Main.ts');
console.log('资源 UUID:', uuid);
```

---

## ⚙️ 配置说明

### package.json 配置项

| 字段 | v2 | v3 | 说明 |
|------|----|----|------|
| 主入口 | `main` | `main` | 插件主文件路径 |
| 菜单 | `main-menu` | `contributions.menu` | 顶部菜单配置 |
| 面板 | `panel` | `panels` | 面板配置 |
| 消息 | - | `contributions.messages` | 消息映射 |
| Inspector | `inspector` | `inspector` | 检查器扩展 |

### TypeScript 配置

- `tsconfig.json` - 基础配置
- `tsconfig.v2.json` - v2 编译配置（输出到 `dist/v2`）
- `tsconfig.v3.json` - v3 编译配置（输出到 `dist/v3`）

---

## 🐛 常见问题

### Q1: 插件没有生效？

**解决方法：**
1. 检查插件是否已启用
2. 尝试重新加载插件/扩展
3. 检查是否已执行编译命令
4. 查看控制台是否有错误信息

### Q2: Inspector 没有显示？

**解决方法：**
1. 确认已选中**文件夹**（不是文件）
2. 检查文件夹内是否有正确命名的 `.{文件夹名}.md` 文件
3. 重新编译插件
4. 重启 Cocos Creator

### Q3: 跨版本开发注意事项？

**建议：**
- 始终使用适配器提供的接口，不要直接调用 `Editor` 对象
- 面板开发时注意 v2 和 v3 的 DOM API 差异
- 测试时在两个版本中都验证功能

### Q4: 如何调试？

**Cocos Creator 2.x:**
- `开发者 → 开发者工具` 打开控制台

**Cocos Creator 3.x:**
- `开发者 → 开发者工具` 打开控制台

在代码中使用：
```typescript
this.editor.log('调试信息');
console.log('更多调试信息');
```

---

## 📚 API 参考

### BasePlugin 基类

| 方法/属性 | 说明 |
|----------|------|
| `load()` | 插件加载时调用（需重写） |
| `unload()` | 插件卸载时调用（需重写） |
| `editor` | 编辑器适配器 |
| `panel` | 面板适配器 |
| `ipc` | 进程通信适配器 |
| `asset` | 资源适配器 |
| `scene` | 场景适配器 |
| `log()` | 输出日志 |
| `warn()` | 输出警告 |
| `error()` | 输出错误 |
| `openPanel()` | 打开面板 |
| `closePanel()` | 关闭面板 |

### 工厂函数

```typescript
// 获取适配器实例
getEditor(): IEditorAdapter
getPanel(): IPanelAdapter
getIPC(): IIPCAdapter
getAsset(): IAssetAdapter
getScene(): ISceneAdapter

// 版本检测
isV2(): boolean
isV3(): boolean
detectVersion(): CocosVersion
```

---

## 📝 版本历史

### v1.0.0 (2026-01-21)

- ✅ 实现跨版本适配器框架
- ✅ 添加 Bundle 路径映射工具
- ✅ 实现资源目录说明 Inspector
- ✅ 支持 Cocos Creator 2.x 和 3.x

---

## 📄 许可证

MIT License

---

## 👥 贡献

欢迎提交 Issue 和 Pull Request！

---

## 📮 联系方式

- 作者：Developer
- 项目：Extensions Tools Universal

---

## 🙏 致谢

感谢 Cocos Creator 社区的支持！
